...
Write-Host $vmname 
write-host $ip
    $vmlocation = "AZ1-VMs" # folder where you to locate VM
    $datastorename = "vsanDatastore" # datastore name.
    $specname = "ubuntu" # name of the specification you have created.
    $subnetmask = "255.255.255.0" #
    $gateway = "192.168.73.254"
    $vmhostname = "esx-01a.corp.internal" # selecting host here instead of cluster
    $templatename = "lu22-t" # this is the template name
    $vsanpolicyName = "Policy-SFTT1" # configure vSAN policy as per the requirements
    ...
    Get-VM $vmname | Get-NetworkAdapter | Set-NetworkAdapter -StartConnected $true -Confirm:$false -OutVariable $null # i have ensure that vNIC is connected. sometimes it is not connected
    get-vm $vmname | Set-VM -MemoryGB 4 -Confirm:$false
    Start-Sleep 10 # little pause
    start-vm $vmname -Confirm:$false -RunAsync -OutVariable $null # start VM
Start-Sleep 10
}